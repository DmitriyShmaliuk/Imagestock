  ...
  >
    <v-container class="reaction" :class="{ active: isReactionSectionOpen }">
      <figure class="dislike-icon" @click="clickDislike">
        <v-icon color="#a0b0ba" :size="28">mdi-thumb-down</v-icon>
        <div class="badge">
          {{ countOfDislike }}
        </div>
      </figure>

      <figure class="like-icon" @click="clickLike">
        <v-icon color="#a0b0ba" :size="28">mdi-thumb-up</v-icon>
        <div class="badge">
          {{ countOfLike }}
        </div>
      </figure>

      <figure class="comments-icon" @click="$emit('open-popup', index)">
        <v-icon color="#a0b0ba" :size="28">mdi-message-reply</v-icon>
        <div class="badge">
          {{ countOfComments }}
        </div>
      </figure>
